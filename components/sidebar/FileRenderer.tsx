...
import React, { createContext, CSSProperties, FocusEvent, HTMLAttributes, MouseEvent, useRef, useState } from "react";
...
export default function FileRenderer({ fileNode, higherIndex = 1 }: { fileNode: fileNode; higherIndex?: number }) {
  const dispatch = useDispatch();
  const precursorNode = new folderNode(fileNode.precursor);
  const { contextMenu, selectedNode, motherNode } = useSelector((state: RootState) => state.motherNode);
  ...
  const fileNameEventListeners: HTMLAttributes<HTMLDivElement> = {
    ...
    onDragStart: (e) => {
      e.dataTransfer.setData("app/drag", fileNode.elementPath);
